...
class ImageUtil 
{
    ...
    /*
        ImageUtil::process_image_anysize(
            
            SITE_ROOT.DIRECTORY_SEPARATOR.'user'.DIRECTORY_SEPARATOR . $t_foldername, 
            $this->mPrimaryCert['name'], 
            600, 
            800
        );
    */
public static function process_image_anysize($dir, $filename, $width, $height) {
    ...
    $image_path_400 = $dir . $image_name . '_'.$width . $ext;
    // $image_path_100 = $dir . $image_name . '_100' . $ext;

    // Create an image that's a maximum of 400x300 pixels
    self::resize_image($image_path, $image_path_400, $width, $height);

    // Create a thumbnail image that's a maximum of 100x100 pixels
    // self::resize_image($image_path, $image_path_100, 100, 100);process_image
}
...
public static function process_image_custom($dir, $filename, $width, $height, $newImageName) {
    // Set up the variables
    $dir = $dir . DIRECTORY_SEPARATOR;
    $i = strrpos($filename, '.');
    $image_name = substr($filename, 0, $i);
    ...
    // Set up the write paths
    $image_path_size = $dir . $newImageName . '_'.$width . $ext;
    // $image_path_100 = $dir . $image_name . '_100' . $ext;

    // Create an image that's a maximum of 400x300 pixels
    self::resize_image($image_path, $image_path_size, $width, $height);
