...
class PsTakePayment implements IPipelineSection 
{
    public function Process($processor)
    {
        ...
        $transaction = array(
            'x_ref_trans_id' => $processor->mOrderInfo['reference'],
            'x_method' => 'CC',
        ...
        {
            //Audit
            $processor->CreateAudit('Funds deducted from customer credit card.', 20402);
        ...
        {
            //Audit
            $processor->CreateAudit('Errors taking funds from customer credit card.', 20403);

            throw new Exception('Credit card take payment failed for order.' . $processor->mOrderInfo['order_id'] . ".\n\n" . 'Data exchanged:' . "\n" . var_export($transaction, true) . "\n" . var_export($response, true));
        }
