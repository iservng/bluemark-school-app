...
class OrderProcessor 
{
    ...
    public function __construct($orderId)
    {
        ...
        $credit_card->LoadEncryptedDataAndDecrypt($this->mCustomerInfo['credit_card']);
    ...
    public function Process()
    {
        ...
        {
            $this->MailAdmin('Order Processing error occurred.', 'Exception: "' . $e->getMessage() . '" on ' . $e->getFile() . ' line ' . $e->getLine(), $this->_mOrderProcessStage);

            $this->CreateAudit('Order Processing error occured.', 10002);

            throw new Exception('Error occured, order aborted. ' . ' Details mailed to administrator');
    ...
    //Sets order authentication code and reference code 
    ...
    //Returns a string that contains the customers address
    public function GetCustomerAddressAsString()
    {
        $new_line = "\n";

        $address_details = $this->mCustomerInfo['name'] . $new_line . $this->mCustomerInfo['address_1'] . $new_line;
    ...
    //Returns a string that contain the order details 
    public function GetOrderAsString($withCustomerDetails = true)
    {
        ...
        foreach($this->mOrderDetailsInfo as $order_detail)
        {
            $order_details .= $order_detail['quantity'] . ' ' . 
                                ...
                                $order_detail['unit_cost'] . ' each, total cost N' . 
                                number_format($order_detail['subtotal'], 2, '.', '') . $new_line;
        ...
        if($this->mOrderInfo['tax_id'] != -1 && $this->mOrderInfo['tax_percentage'] != 0.00)
        {
            ...
            $order_details .= 'Tax: ' . $this->mOrderInfo['tax_type'] . ', N' . 
                                    number_format($tax_amount, 2, '.', '') . $new_line;

            $total_cost += $tax_amount;
        }

        $order_details .= $new_line . 'Total order cost: N' . 
