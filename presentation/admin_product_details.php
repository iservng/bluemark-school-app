<?php
//Class that deal with product administration
class AdminProductDetails
{
    //Public attributes available in smarty template
    public $mProduct;
    ...
    public function __construct()
    {
        ...
        $this->mLinkProductDetailsAdmin = Link::ToProductAdmin($this->_mDepartmentId, $this->_mCategoryId, $this->_mProductId);
    ...
    public function init()
    {
        ...
        {
            /*Chech whether we have write permision on the 
            product_images folder*/
            if(!is_writeable(SITE_ROOT . '/product_images'))
            {
                echo "Cant write to the product_images forlder";
                exit();
            }
            ...
            //If the error code is 0, the file uploaded ok
            if($_FILES['Image2Upload']['error'] == 0)
            {
                ...
                //Update the products information in the database
                Catalog::SetImage($this->_mProductId, $_FILES['Image2Upload']['name']);
            }
        ...
        //If assign the product to another category...
        ...
        for($i = 0; $i < count($product_attributes); $i++)
            $this->mProductAttributes[$product_attributes[$i]['attribute_value_id']] = $product_attributes[$i]['attribute_name'] . ': ' . $product_attributes[$i]['attribute_value']; //Remind me to check you out
        ...
        //Show the categories the product belong to 
