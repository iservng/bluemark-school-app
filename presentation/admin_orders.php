...
/*Presentation tier class that susport order administration functionality*/
class AdminOrders
{
    //Public variable available in smart template 
    public $mOrders;
    public $mStartDate;
    public $mEndDate;
    ...
    public function __construct()
    {
        ...
        session variable; it will be used to create the ff
    ...
    {
        //If the "Show the most recent x order" filters in action ... 
        if(isset($_GET['submitMostRecent']))
        {
            //if the record count value is not a valid integer, display error hhh
        ...
        /*If the "Show all records created between data_1 and date_2" 
        filter is in action ... */
        if(isset($_GET['submitBetweenDates']))
        {
            ...
            if(($this->mStartDate == '') || ($timestamp = strtotime($this->mStartDate)) == -1)
                $this->mErrorMessage .= 'This start date is invalid. ';
            else 
                //Transform date to YYYY/MM/DD HH:MM:SS formate
                $this->mStartDate = strftime('%Y/%m/%d %H:%M:%S', strtotime($this->mStartDate));


            //Check if  the end date is in accept formate
            if(($this->mEndDate == '') || ($timestamp = strtotime($this->mEndDate)) == -1)
            {
                $this->mErrorMessage .= 'This end date is invalid.';
            }
            else 
                //Tranform the data to YYY/MM/DD HH:MM:SS format
                $this->mEndDate = strftime('%Y/%m/%d %H:%M:%S', strtotime($this->mEndDate));


            //Check if the start date is more recent than the end data
            if((empty($this->mErrorMessage)) && (strtotime($this->mStartDate) > strtotime($this->mEndDate)))
                $this->mErrorMessage = 'The start date should not be more recent than the end date';
        ...
        //If the "Get order by ID " filter is in action //dont
        ...
        if(is_array($this->mOrders) && count($this->mOrders) == 0)
            $this->mErrorMessage = 'No order found matching your search criteria!';

        
        //Build View Detail link
