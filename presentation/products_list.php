...
class ProductsList
{
    //Public variales to be rad from smarty template
    public $mPage = 1;
    public $mrTotalPages;
    ...
    {
        //Retrives the search string and Allwords from the query string
        ...
        //Get DepartmentId from query string cassting it to string
        if(isset($_GET['DepartmentId']))
            $this->_mDepartmentId = (int)filter_input(INPUT_GET, 'DepartmentId');

        if(isset($_GET['CategoryId']))
            $this->_mCategoryId = (int)$_GET['CategoryId'];

        //Get page number from query string cating it to int
        if(isset($_GET['Page']))
            $this->mPage = (int)$_GET['Page'];

        if($this->mPage < 1)
            trigger_error('Incorect Page Value');
    ...
    public function init()
    {
        ...
        if(isset($this->mSearchString))
        {
            ...
            if(count($search_results['ignored_words']) > 0)
                $this->mSearchDescription .= '<p class="description"> Ignored words: <font class="words">'
        ...
        /*If browsing categories, get the list of products by calling
        ...
        */
        ...
        /*404 redirect if the page number is larger than the tottal number of pages*/
        if($this->mPage > $this->mrTotalPages && !empty($this->mrTotalPages))
        {
            ...
            //clean the output buffer and stop execution
            flush();
            ob_flush();
            ob_end_clean();
            exit();
        ...
        //Build links for products details page
