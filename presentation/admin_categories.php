...
class AdminCategories
{
    ...
    public function __construct()
    {
        ...
        foreach($_POST as $key => $value)
            ...
            if(substr($key, 0, 6) == 'submit')
            {
                ...
                button name. eg strpos('submit_edit_cat_1', '_') is 16*/
                $last_underscore = strrpos($key, '_');

                /*Get the scope of submit button
                (eg 'edit_cat' from 'submit_edit_cat_1')*/
                $this->_mAction = substr($key, strlen('submit_'), $last_underscore - strlen('submit_'));

                /*Get the category id targeted by the submit button
                (the number at the end of the button name)
                eg '1' from 'submit_edit_cat_1'*/
    ...
    public function init()
    {
        ...
        if($this->_mAction == 'update_cat')
        {
            ...
            if($category_name == null)
                $this->mErrorMessage = 'Category name is required';
