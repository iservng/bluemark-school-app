...
class CartDetails 
{
    //Public variables aailable in smarty template 
    public $mCartProducts;
    public $mSavedCartProducts;
    public $mTotalAmount;
    public $mIsCartNowEmpty = 0; //Is the chopping cart empty ?
    ...
    public function __construct()
    {
            ...
            $this->_mCartAction == MOVE_PRODUCT_TO_CART)
        {
            if(isset($_GET['ItemId']))
                ...
                trigger_error('ItemId must be set for this type of request', E_USER_ERROR);
        }
    ...
    public function init()
    {
        switch ($this->_mCartAction) 
        {
            case ADD_PRODUCT:
                ...
                {
                    //If there fields starting with "attr_" in the post array
                    if(substr($key, 0, 5) == 'attr_')
                    {
                        //Get the select attribute name and value
        ...
        /*Calculate the total amount for the shoppin cart
        befor applicable taxes and/or shipping*/
       ...
       if($this->mTotalAmount != 0 && Customer::IsAuthenticated())
       {
        ...
        //Get product recommendation for the shopping cart
